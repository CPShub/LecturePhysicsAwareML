...
def plot_forces(Qf):
    ...
    fig.suptitle('Time series of internal forces', fontsize=16, y=.995)
    plt.tight_layout()
...
def plot_stiffnesses(QKQ):
    ...
    fig.suptitle('Time series of internal stiffnesses', fontsize=16, y=.995)
    plt.tight_layout()
...
def plot_predictions(x, Qf, Qf_pred, QKQ, QKQ_pred, E=None):
    ...
    for i, ax in enumerate(axs.reshape(-1)):
        ...
        ax.set_xlim(x_lim)
        ax.set_ylim(y_lim)
    fig.suptitle('Correlation of internal forces', fontsize=16,  y=.995)
    plt.tight_layout()
    ...
    for i, ax in enumerate(axs.reshape(-1)):
        ...
        ax.set_xlim(x_lim)
        ax.set_ylim(y_lim)
    fig.suptitle('Correlation of internal stiffnesses', fontsize=16,  y=.995)
    plt.tight_layout()
    ...
    if not E == None:
        ...
        ax.set_ylabel(f'E')
        ax.legend()
        fig.suptitle('Time series of internal energy', fontsize=16,  y=.995)
        plt.tight_layout()
    ...
    for i in range(n):
        ...
        axs[i].legend()
    fig.suptitle('Time series of function values', fontsize=16,  y=.995)
    plt.tight_layout()
