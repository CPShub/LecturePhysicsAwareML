...
def make_layer(m_type, params, **kwargs):
    ...
    cf = {"Maxwell": MaxwellAnalyticalCell, "Naive": NaiveRNNCell, "GSM": GSMCell}
...
class MaxwellAnalyticalCell(layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        eps_N = tf.gather(inputs, [0], axis=-1)
        hs_N = tf.gather(inputs, [1], axis=-1)
        gamma_N = states[0]
...
class NaiveRNNCell(layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        sig_N = tf.gather(y, [0], axis=-1)
        gamma_n = tf.gather(y, [1], axis=-1)
...
class GSMCell(layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        sig_N = tf.gather(dedgamma, [0], axis=-1)
        gamma_n = gamma_N - hs_N * self.eta ** (-1) * tf.gather(dedgamma, [1], axis=-1)
